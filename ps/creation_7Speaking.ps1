...
SQLRequest ("Select nom, mail FROM plateforme WHERE nom = '7Speaking';")
$mail7Speaking = $result.Rows[0]["mail"]
...
Add-Content -Path 7Speaking.csv  -Value '"Nom","Prenom","email","ID interne","Date debut de formation","duree"'  
  ...
  $stagiaires7Sspeaking | foreach { Add-Content -Path  7Speaking.csv -Value $_ }


#Envoi du mail avec le CSV
Send-MailMessage -From "Eric Persan epersan@eni-ecole.fr" -To $mail7Speaking -Subject "ENI Ecole - Creation de comptes" -Body "Bonjour, veuillez trouver ci joint le fichier CSV contenant les comptes à créer" -Attachments "7Speaking.csv" -SmtpServer $SmtpENI